...
const ReportesModel = {
  getAll: async () => {
    const result = await pool.query(`
      SELECT r.*, u.nombre AS nombre_usuario, u.area AS area_usuario,
  ...
  update: async (id, { descripcion, estado, prioridad, id_equipo }) => {
    // Intentar con prioridad e id_equipo
    try {
      const result = await pool.query(
        `UPDATE reportes_trabajador
         SET descripcion=$1, estado=$2, prioridad=$3, id_equipo=$4
         WHERE id_reporte_trabajador=$5 RETURNING *`,
        [descripcion, estado, prioridad || 'Baja', id_equipo || null, id]
      );
      return result.rows[0];
    } catch (e) {
      // Fallback: actualizar solo descripcion y estado (si prioridad no existe en BD aún)
      const result = await pool.query(
        `UPDATE reportes_trabajador
         SET descripcion=$1, estado=$2
         WHERE id_reporte_trabajador=$3 RETURNING *`,
